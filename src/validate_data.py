...
def get_random_proposals_with_metadata(
    db_paths: Sequence[Path], num_proposals: int = 10, seed: int = 42
    ...
    sampled_proposals = global_proposals.sort("proposal_id").sample(
        n=num_proposals, shuffle=True, seed=seed
    )
    ...
    for path in db_paths:
        with sqlite3.connect(str(path)) as conn:
            ...
            db_projects = raw_projects.join(
                target_project_ids, on="project_id", how="inner"
            )
            if db_projects.is_empty():
                continue

            # Load matching, trimmed-down downstream metadata tables
            proposals = sampled_proposals.join(
                db_projects.select("project_id"), on="project_id", how="inner"
            )

            revisions = (
                _safe_read_table(conn, "ProposalRevision")
                .join(target_ids, on=["project_id", "proposal_id"], how="inner")
                .sort("proposal_id")
            )
            revision_authors = (
                _safe_read_table(conn, "ProposalRevisionAuthor")
                .join(target_ids, on=["project_id", "proposal_id"], how="inner")
                .sort("proposal_id")
            )
            comments = (
                _safe_read_table(conn, "Comment")
                .join(target_ids, on=["project_id", "proposal_id"], how="inner")
                .sort("proposal_id")
            )
            statuses = (
                _safe_read_table(conn, "ProposalStatus")
                .join(target_ids, on=["project_id", "proposal_id"], how="inner")
                .sort("proposal_id")
            )
            related_proposals = (
                _safe_read_table(conn, "RelatedProposal")
                .join(target_ids, on=["project_id", "proposal_id"], how="inner")
                .sort("proposal_id")
            )
            ...
            persons = _safe_read_table(conn, "Person").join(
                people_ids, on="person_id", how="inner"
            )
            person_identifiers = _safe_read_table(conn, "PersonIdentifier").join(
                people_ids, on="person_id", how="inner"
            )
            affiliations = _safe_read_table(conn, "Affiliation").join(
                people_ids, on="person_id", how="inner"
            )

            org_ids = affiliations.select("organisation_id").drop_nulls().unique()
            organisations = _safe_read_table(conn, "Organisation").join(
                org_ids, on="organisation_id", how="inner"
            )
            ...
            for row in db_projects.unique(subset=["project_id"]).iter_rows(named=True):
                ...
                contexts.append(
                    IndividualProjectContext(
                        ...
                        proposal_revisions=revisions.filter(
                            pl.col("project_id") == p_id
                        ),
                        proposal_revision_authors=revision_authors.filter(
                            pl.col("project_id") == p_id
                        ),
                        ...
                        related_proposals=related_proposals.filter(
                            pl.col("project_id") == p_id
                        ),
                    )
                )
...
if __name__ == "__main__":
    ...
    DATABASE_PATHS = [
        Path(
            r"C:\Users\risse\University\CSE3000 Research Project\proposal-governance-analysis\data\proposals\cplusplus_proposals_2026-06-02.sqlite3"
        )
    ]
    ...
    if not any(p.exists() for p in DATABASE_PATHS):
        print(
            "Please configure valid database path strings in the script execution block."
        )
    else:
        sampled_contexts = get_random_proposals_with_metadata(
            db_paths=DATABASE_PATHS, num_proposals=10, seed=42
        )
