...
def compute_independence_hhi(
    ctx: IndividualProjectContext, window_size: int, mode: str, n_bootstraps: int = 200
...
def compute_centralization_metrics(
    ...
    window_size: int,
    mode: str,
    n_bootstraps: int = 40,  # Network analysis is heavier; scale bootstraps carefully
) -> Union[Dict[str, float], Dict[int, Dict[str, float]]]:
...
def get_governance_statistics(
    ...
    for ctx in projects:
        ...
        for domain_key, (compute_fn, attribute_name) in computation_registry.items():
            ...
            yearly_raw_dict = compute_fn(ctx, window_size=1, mode="all_windows")
            ...
            pooled_raw_dict = compute_fn(ctx, window_size=5, mode="most_recent")
            setattr(
