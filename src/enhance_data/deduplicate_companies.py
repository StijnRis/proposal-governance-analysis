...
from src.dataloader import TABLE_SCHEMAS, IndividualProjectContext
...
def merge_duplicate_companies_in_contexts(
    ...
    for alias, master in list(alias_dict.items()):
        if master in alias_dict and alias_dict[master] != master:
            ...
            print(f"Warning: Master name '{master}' is an alias for '{ultimate_master}'. Updating map.")
    ...
    for ctx in contexts:
        ...
        normalized = ctx.organisations.with_columns(
            ...
            .str.replace_all(r"[^a-z ]", " ")  # Replaces symbols like '@' and '-' with spaces
        ...
        master_map = normalized.group_by("norm_key").agg(
            ...
            pl.col("norm_key").first().alias("master_name"),  # FIX: Save the normalized name here
        )
        ...
        if not ctx.affiliations.is_empty():
            id_bridge = resolved.select(["organisation_id", "master_id"]).unique()
            ctx.affiliations = (
                ctx.affiliations.join(id_bridge, on="organisation_id", how="left")
                .with_columns(pl.col("master_id").fill_null(pl.col("organisation_id")).alias("organisation_id"))
                .drop("master_id")
                .unique()
    ...
    print(f"Deduplication complete: {original_count} original companies reduced to {clean_count} unique companies.")

    # 6. Save text outputs
    os.makedirs("output", exist_ok=True)
    for filename, dataset in [("companies_original.txt", all_original_companies), ("companies.txt", all_clean_companies)]:
        if dataset:
            sorted_data = sorted([str(name).strip() for name in dataset if str(name).strip()])
