...
cur = conn.cursor()
cur.execute(
    "SELECT proposal_id, normalised_status, created_at FROM ProposalStatus WHERE project_id = ? ORDER BY proposal_id LIMIT 500",
    (PROJECT_ID,),
)
rows = cur.fetchall()
conn.close()
...
print(f"Wrote samples to {OUT.name}")
